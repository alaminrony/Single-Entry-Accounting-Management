...
<div class="content-wrapper">
    ...
    <section class="content-header">
        <div class="container-fluid">
            <div class="row mb-2">
                <div class="col-sm-6">
                    <h1>@lang('lang.EDIT_INVOICE')  for {{$issueArr[request()->route('issue_id')] ?? ''}}</h1>
                </div>
            </div>
        </div>
    </section>
                ...
                <div class="col-md-12">
                    <div class="card card-secondary">
                        <div class="card-header">
                            <h3 class="card-title">@lang('lang.EDIT_INVOICE')</h3>
                        ...
                        {!!Form::open(['route'=>['invoice.update',request()->route('issue_id')],'class'=>'form-horizontal','enctype' => 'multipart/form-data'])!!}
                        ...
                        <div class="card-body">
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        {!!Form::text('invoice_code',$target->invoice_code,['class'=>'form-control','id'=>'invoice_code','readonly'])!!}
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <div class="input-group date" id="visa_issue_date" data-target-input="nearest">
                                            <input type="text" name='inv_date' class="form-control datetimepicker-input" data-target="#visa_issue_date" value="{{$target->inv_date ?? old('inv_date')}}" placeholder="yyyy/mm/yy" id="visaIssueDate"/>
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        <div class="input-group date" id="passport_issue_date" data-target-input="nearest">
                                            <input type="text" name='due_date' class="form-control datetimepicker-input" data-target="#passport_issue_date" value="{{$target->due_date ?? old('due_date')}}" placeholder="yyyy/mm/yy" id="visaIssueDate"/>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        {!!Form::select('select_issue_id',$issueArr??'',$target->issue_id ?? old('select_issue_id'),['class'=>'form-control select2','id'=>'selectIssueId'])!!}
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        {!!Form::select('customer_code',$cusCode??'',$target->customer_code ?? old('customer_code'),['class'=>'form-control select2','id'=>'customerCode'])!!}
                            ...
                            <div class="form-group row">
                                ...
                                <div class="input-group date col-md-6"  data-target-input="nearest">
                                    {!!Form::select('bill_to',$users,$target->bill_to ?? '',['class'=>'form-control select2','id'=>'bill_to']) !!}
                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        {!!Form::text('billing_street',$target->billing_street ?? old('billing_street'),['class'=>'form-control','id'=>'billing_street','placeholder'=>"Enter billing street"])!!}
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        {!!Form::text('billing_city',$target->billing_city ?? old('billing_city'),['class'=>'form-control','id'=>'billing_city','placeholder'=>"Enter billing city"])!!}
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        ...
                                        {!!Form::text('billing_zip',$target->billing_zip ?? old('billing_zip'),['class'=>'form-control','id'=>'billing_zip','placeholder'=>"Enter billing zip"])!!}
                                    ...
                                    <div class = "form-group">
                                        <div class ="table-responsive">
                                            <table class ="table table-bordered" id="dynamic_field">
                                                ...
                                                <tbody>
                                                    <?php $rowCount = count($target->details) ?>

                                                    @if($rowCount > 0)
                                                    @foreach($target->details as $key => $details)
                                                    <?php $key++; ?>
                                                    <tr id="row{{$key}}">
                                                        <td style="display: none;"><input  name="dynamic_product_id[]" value="{{$details->product_id ?? ''}}" id="pId-{{$key}}"></td>
                                                        <td><textarea name="product_name[]" class="form-control" rows="2" cols="30" style="resize:none;" id="dProId-{{$key}}"  readonly="readonly">{{$details->product_name ?? ''}}</textarea></td>
                                                        <td><textarea name="description[]" class="form-control" rows="2" cols="30" style="resize:none;">{{$details->description ?? ''}}</textarea></td>
                                                        <td><input type="number" name="quantity[]" value="{{$details->quantity ?? ''}}" placeholder="Enter quantity" class="form-control quantity" id="qty-{{$key}}" target-qty="{{$key}}"></td>
                                                        <td><input type="number" name="unit_price[]" value="{{$details->unit_price ?? ''}}" placeholder="Enter unit price" class="form-control" id="productPrice-{{$key}}"></td>
                                                        <td><input type="text" name="tax[]" value="{{$details->tax ?? ''}}" class="form-control tax"  placeholder="Enter tax"  id="tax-{{$key}}" target-tax="{{$key}}"></td>
                                                        <td><input type="text" name="total[]"  value="{{$details->total ?? ''}}" class="form-control"  placeholder="Enter total"  id="total-{{$key}}" required=""></td>
                                                        @if($key-1 == 0)
                                                        <td><button type ="button" name="add" id="add" class="btn btn-success"><i class="fa fa-plus font-red"></i></button></td>
                                                        @else
                                                        <td><button type ="button"  name="remove" id="{{$key}}" class="btn btn-danger btn_remove"><i class="fa fa-times font-red"></i></button></td>
                                                        @endif
                                                    </tr>
                                                    @endforeach
                                                    @else
                                                    <tr class="numbar">
                                                        ...
                                                        <td><input type="number" name="unit_price[]" value="" placeholder="Enter unit price" class="form-control" id="productPrice-1"></td>
                                                        <td><input type="text" name="tax[]" value="" class="form-control tax"  placeholder="Enter tax"  id="tax-1" target-tax="1"></td>
                                                        <td><input type="text" name="total[]"  value="" class="form-control"  placeholder="Enter total"  id="total-1" required=""></td>
                                                        <td><button type ="button" name="add" id="add" class="btn btn-success"><i class="fa fa-plus font-red"></i></button></td>
                                                    </tr>
                                                    @endif
                                                <tbody>
                                                <tfoot>
                                                    ...
                                                    <tr>
                                                        <td colspan="3"></td>
                                                        <td colspan="2"><input type="text" class="form-control" value="0" placeholder="Enter Discount(%)"  id="discountAmount"></td>
                            ...
                            <input type="hidden" name="fieldCount" value="{{$rowCount ?? 1}}" id="fieldCount">
                            <input type="hidden" name="fieldCountRemove" value="{{$rowCount ?? 1}}" id="fieldCountRemove">

                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label>@lang('lang.NOTE')</label>
                                        {!! Form::textarea('invoicenote',$target->invoicenote ?? '', ['id' => 'invoicenote', 'rows' => 4, 'cols' => 54, 'style' => 'resize:none','class'=>'form-control']) !!}
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        {!! Form::textarea('terms', $target->terms ?? '', ['id' => 'note', 'rows' => 4, 'cols' => 54, 'style' => 'resize:none','class'=>'form-control']) !!}
                                        @if($errors->has('terms'))
...
<script type="text/javascript">
    ...
    $('#subTotal').text(<?php echo $target->sub_total ?? 0; ?>);
    $('#subTotalHiddenValue').val(<?php echo $target->sub_total ?? 0; ?>);

    $("#totalVal").text(<?php echo $target->total_amount ?? 0; ?>);
    $("#totalHiddenValue").val(<?php echo $target->total_amount ?? 0; ?>);

    $("#discountAmount").val(<?php echo $target->discount_percent ?? 0; ?>);
    $("#discountVal").text(<?php echo $target->discount_total ?? 0; ?>);
    $("#discountHiddenValue").val(<?php echo $target->discount_total ?? 0; ?>);
    $("#discountPercentHiddenValue").val(<?php echo $target->discount_percent ?? 0; ?>);
...
//        alert(button_id);return false;
        ...
        let fCountRemove = parseInt($('#fieldCountRemove').val());
        $('#fieldCountRemove').val(fCountRemove - 1);
        totalSum();
...
//            return false;
            $("#dProId-" + fCountRemove).val(productArr[productId]);
            $("#pId-" + fCountRemove).val(productId);
            $("#productPrice-" + fCountRemove).val(productPriceArr[productId]);
        }
    ...
    $(document).on('keyup blur', '#discountAmount', function () {
...
//                    customerCode
    ...
    function totalSum() {
        ...
        $("input[name^='total']").each(function () {
            test_qty += parseFloat($(this).val(), 10)
        })
        let discountPer = $('#discountAmount').val();
        if (discountPer != '') {
            var discount = discountPer;
            var subTotal = test_qty;
